...
export default function Success() {
  ...
  return (
    <Container>
      ...
      <Item.Group>
        {cart.map((item) => (
          <SuccessItem item={item} />
        ))}
      </Item.Group>
      <Header as="h2">Total Amount Paid </Header>
      <Header as="h3">${amount}</Header>
      <Header as="h4">
        The receipt has been sent to the following email address -{" "}
        {customerState.email}
      </Header>
      <Header as="h4">
        The items will be delivered to {customerState.card.address_line1},{" "}
        {customerState.card.address_city}{" "}
      </Header>
      <Link to="/">
