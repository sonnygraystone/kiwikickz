...
function UserQuestion() {
  ...
  const questions = allQuestions.filter(
    (question) =>
      question.productId == Number(id) && question.status == "answered"
  );

  const defaultState = { product_id: id, question: "", status: "unanswered" };
  ...
  return (
    <>
      ...
      <ul className="no_bullets">
        {questions.map((ele) => (
          <div key={ele.id}>
            <br />
            <li as="h5">
              <Header as="h5">Q :{ele.question}</Header>
              <p>A : {ele.answer}</p>
            </li>
          </div>
        ))}
      ...
      <Container className="question-box">
        ...
        {asked ? (
          <p className="qsubmit">
            Thanks for your question. Admins will respond at their neareast
            convenience
          </p>
        ) : null}
      </Container>
