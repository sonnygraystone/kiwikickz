...
function QuestionAdmin() {
  ...
  const handleDelete = (e, id) => {
    ...
    if (confirm(text) == true) {
      dispatch(deleteQuestionThunk({ id: id })).then(
        (confirmationString) => {}
      );
    } else {
              ...
              <li as="h5">
                <Header>
                  ...
                  <span className="notbold">
                    ({ele.createdAt.slice(8, 10)}.{ele.createdAt.slice(5, 7)}.
                    {ele.createdAt.slice(2, 4)} 🕒 {ele.createdAt.slice(11, 16)}
                  </span>{" "}
                  ){" "}
                  <a
                    href={`/product/${returnURLId(ele.productId)}`}
                    target="_blank"
                  >
                    {returnProdName(ele.productId)}
                  </a>
                </Header>
                <Form
                  type="submit"
